...
class LlamaBlock(nn.Module):
    ...
    def forward(self, x, position_embeddings, return_attention_map):
        attention_output, attention_map_1 = self.attn(self.attn_norm(x), position_embeddings, return_attention_map)
        h = x + attention_output
        attention_output_2, attention_map_2 = self.ffn(self.ffn_norm(h), position_embeddings, return_attention_map)
        x = h + attention_output_2
        return x, h, [attention_map_1, attention_map_2]
...
class Llama(nn.Module):
    ...
    def forward(self, x, record_mode=False):
        ...
        for layer in self.layers:
            ...
            if record_mode:
                all_attention_maps.extend(attention_map)
