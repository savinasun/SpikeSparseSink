...
class Attention(nn.Module):
    def __init__(self, hidden_size, num_heads, num_kv_heads, head_dim, gated_attention_type, max_sequence_length):
        super().__init__()
        self.hidden_size = hidden_size
        ...
        if gated_attention_type == "standard":
            self.gate = nn.Parameter(torch.empty(num_heads * self.head_dim))
        elif gated_attention_type == "head":
            self.gate = nn.Parameter(torch.empty(num_heads))
        elif gated_attention_type == "token":
            self.gate = nn.Parameter(torch.empty(max_sequence_length, num_heads, self.head_dim))
        elif gated_attention_type is not None:
            raise ValueError(f"Unsupported gated_attention_type: {gated_attention_type}")

    def forward(self, x, position_embeddings, return_attention_map=False):
        ...
        if self.gated_attention_type == "standard":
            g = self.gate.unsqueeze(0).unsqueeze(0) # shape (1, 1, num_heads * head_dim)
            a = a * torch.sigmoid(g)
        elif self.gated_attention_type == "head":
            g = self.gate.unsqueeze(0).unsqueeze(0).unsqueeze(-1) # shape (1, 1, num_heads, 1)
            ...
            g = self.gate.unsqueeze(0)[:,:sequence_length].view(1, sequence_length, -1) # shape (1, max_sequence_length, num_heads * head_dim)
            a = a * torch.sigmoid(g)
        else:
...
class LlamaBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, num_kv_heads, head_dim, intermediate_size, norm_eps, gated_attention_type, max_sequence_length):
        super().__init__()
    ...
    def forward(self, x, position_embeddings, return_attention_map):
        attention_output, attention_map = self.attn(self.attn_norm(x), position_embeddings, return_attention_map)
...
class Llama(nn.Module):
    ...
    def forward(self, x, record_mode=False):
        ...
        for layer in self.layers:
            h, hidden_representation, attention_map = layer(h, position_embeddings, return_attention_map=record_mode)
            if record_mode:
