...
class Attention(nn.Module):
    def __init__(self, hidden_size, num_heads, num_kv_heads, head_dim, norm_eps):
        super().__init__()
        self.hidden_size = hidden_size
        ...
        self.q_norm = RMSNorm(num_heads * head_dim, eps=norm_eps)
        self.k_norm = RMSNorm(num_kv_heads * head_dim, eps=norm_eps)
        
    def forward(self, x, position_embeddings, return_attention_map=False):
        batch_size, sequence_length, _ = x.shape
        q = self.q_norm(self.wq(x)).view(batch_size, sequence_length, -1, self.head_dim).transpose(1, 2) # (B, num_heads, N, head_dim)
        k = self.k_norm(self.wk(x)).view(batch_size, sequence_length, -1, self.head_dim).transpose(1, 2) # (B, num_kv_heads, N, head_dim)
...
class LlamaBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, num_kv_heads, head_dim, intermediate_size, norm_eps):
        super().__init__()
        self.attn = Attention(
    ...
    def forward(self, x, position_embeddings, return_attention_map):
        attention_output, attention_map = self.attn(x, position_embeddings, return_attention_map)
        h = x + self.attn_norm(attention_output)
        x = h + self.ffn_norm(self.ffn(h))
