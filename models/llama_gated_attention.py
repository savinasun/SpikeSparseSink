...
class Attention(nn.Module):
    ...
    def forward(self, x, position_embeddings, return_attention_map=False):
        ...
        if self.gated_attention_type == "standard":
            g = self.wg(x) # shape (batch_size, sequence_length, num_heads * head_dim)
            a = a * torch.sigmoid(g)
        elif self.gated_attention_type == "head":
            g = self.wg(x).view(batch_size, sequence_length, self.num_heads, 1) # shape (batch_size, sequence_length, num_heads, 1)
            ...
            g = self.wg(x).view(batch_size, sequence_length, 1) # shape (batch_size, sequence_length, 1)
            a = a * torch.sigmoid(g)
        else:
